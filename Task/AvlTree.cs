...
//Source: https://bitlush.com/blog/efficient-avl-tree-in-c-sharp
    public class AvlTree<TKey, TValue> : IEnumerable<TValue>
    {
        ...
        public AvlNode Insert(TKey key, TValue value)
        {
            AvlNode inserted = null;

            if (_root == null)
            {
                _root = inserted = new AvlNode {Key = key, Value = value};
                return inserted;
            }
            AvlNode node = _root;
            ...
            while (node != null)
            {
                ...
                if (compare < 0)
                {
                    ...
                    if (left == null)
                    {
                        node.Left = inserted = new AvlNode {Key = key, Value = value, Parent = node};
                ...
                else if (compare > 0)
                {
                    ...
                    if (right == null)
                    {
                        node.Right = inserted = new AvlNode {Key = key, Value = value, Parent = node};
        ...
        public AvlNode SearchOrLess(TKey key)
        {
            AvlNode node = _root;
            AvlNode lastLess = null;
            while (node != null)
            {
                if (_comparer.Compare(key, node.Key) < 0)
                {
            ...
            return lastLess;//default(TValue);
        }

        public AvlNode SearchOrGreater(TKey key)
        {
            AvlNode node = _root;
            AvlNode lastGreater = null;
            while (node != null)
            {
                if (_comparer.Compare(key, node.Key) < 0)
                {
            ...
            return lastGreater;//default(TValue);
        }
